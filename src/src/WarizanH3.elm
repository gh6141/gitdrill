...
type Msg
    =  Next | Newmon (Int,Int) |Amari (Int,(Int,Int)) | Btn Int |Btn2 Sd |Kirikae |Suj (Ichi,Kt) |Jido |Amarif
  ...
  case msg of
   ...
   Newmon tpl ->
      let
           m1x=fst tpl
           m2x=snd tpl
           ahenkan amari = (amari,(m1x,m2x))
           monGA = Random.map  ahenkan 
              ( 
                 if model.amariflg then
                  (Random.int 1 (m2x-1) ) 
                 else
                  (Random.int 0 0)
              )
      in
      (model, Random.generate Amari monGA)
         ...
         ,sagyo=
            let
               ...
               fff idx chh=  { 
                  ...
                  ,iy=2* iix+2 } 
               sagl=List.indexedMap  fff  (String.toList (if (model.amariflg && (String.length anso)==(iix+1)) then (String.dropRight 1 tmp) else tmp) )
               --xx=Debug.log "sagl=" sagl
            in
             sagl
   ...
   Amarif ->
     ({model|amariflg=if model.amariflg then False else True},Cmd.none)
